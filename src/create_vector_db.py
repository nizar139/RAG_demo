...
def markdown_to_chunks(file_path: str, header_levels: list = None, chunk_size: int = 256, chunk_overlap: int = 50) -> List[Document]:
    """
...
def main():
    parser = argparse.ArgumentParser(description="Create a vector database from markdown files.")
    parser.add_argument("--docs_path", type=str, required=True, help="Path to the markdown documents.")
    parser.add_argument("--save_path", type=str, required=True, help="Path to save the vector database.")
    ...
    parser.add_argument("--embedding_model", type=str, default=EMBED_MODEL, help="Embedding model to use.")
    ...
    for docs in glob.iglob(f"{args.docs_path}/*.md"):
        chunks = markdown_to_chunks(docs, chunk_size=args.chunk_size, chunk_overlap=args.chunk_overlap)
        _ = vector_store.add_documents(documents=chunks)
        
    results = vector_store.similarity_search_with_score("If the QRC fails during boot, what is the expected system behavior and recommended recovery steps?")
